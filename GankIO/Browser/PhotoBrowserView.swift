...
class PhotoBrowserView: UIView, UICollectionViewDelegate, UIScrollViewDelegate, UICollectionViewDataSource, BrowserCellDelagate {
    ...
    func setupView(frame: CGRect){
        ...
        myLayout!.itemSize = CGSize(width: frame.width, height: heightUnit!*(self.count - 2))
        myLayout!.scrollDirection = UICollectionViewScrollDirection.Horizontal
    
        collectionView = UICollectionView(frame: CGRect(x: 0, y: heightUnit!, width: frame.width, height: heightUnit!*(self.count - 2)), collectionViewLayout: myLayout!)
    ...
    func addDownloadButton(frame: CGRect){
        downloadButton = UIButton(frame: CGRect(x: frame.width / 2 - 13, y: heightUnit!*(count - 1), width: 26, height: 26))
